    ...
    if CROWD_THRESHOLDS_FILE.exists():
        try:
            with open(CROWD_THRESHOLDS_FILE, "r") as f:
                raw = json.load(f)
            # JSON keys are always strings; normalize to int zone ids.
            CROWD_THRESHOLDS = {int(k): int(v) for k, v in (raw or {}).items()}
...
def _save_crowd_thresholds():
    ...
    try:
        ...
        serializable = {str(k): int(v) for k, v in CROWD_THRESHOLDS.items()}
...
SESSIONS_FILE = (ALERTS_DIR / "sessions.json").resolve()


def _load_sessions():
    """Load session tokens from disk (survives backend restart)."""
    global LIFEGUARD_SESSIONS
    if SESSIONS_FILE.exists():
        try:
            with open(SESSIONS_FILE, "r") as f:
                LIFEGUARD_SESSIONS = json.load(f)
            print(f"[lifeguard] Loaded {len(LIFEGUARD_SESSIONS)} sessions from {SESSIONS_FILE}")
        except Exception as e:
            print(f"[lifeguard] Error loading sessions: {e}")
            LIFEGUARD_SESSIONS = {}
...
def _save_sessions():
    ...
    try:
        ...
        with open(SESSIONS_FILE, "w") as f:
            json.dump(LIFEGUARD_SESSIONS, f, indent=2)
    except Exception as e:
        print(f"[lifeguard] Error saving sessions: {e}")


def _issue_session(lg_id: str) -> str:
    """Create a new session token for a lifeguard."""
...
def _resolve_lifeguard_token(req):
    """Return (lifeguard_id, token) from Authorization: Bearer header."""
    ...
    if not token:
        return None, None
    lg_id = LIFEGUARD_SESSIONS.get(token)
    if not lg_id or lg_id not in LIFEGUARDS:
        return None, None
    return lg_id, token


def _lifeguard_assigned_zones(lg_id: str) -> List[int]:
    """Return assigned zone IDs. Empty list means all zones."""
    with _lifeguard_lock:
        lg = LIFEGUARDS.get(lg_id)
        if not lg:
            return []
        zones = lg.get("zones", [])
        return [int(z) for z in zones] if zones else []


def _zone_allowed_for_lifeguard(lg_id: str, zone_id: int) -> bool:
    """True if lifeguard may access this zone (empty assignment = all zones)."""
    assigned = _lifeguard_assigned_zones(lg_id)
    if not assigned:
        return True
    return int(zone_id) in assigned


def _filter_zone_items_for_lifeguard(items: List[dict], lg_id: str) -> List[dict]:
    assigned = _lifeguard_assigned_zones(lg_id)
    if not assigned:
        return items
    return [item for item in items if int(item.get("id", -1)) in assigned]


def _lifeguard_public_profile(lg_id: str, lg: dict, session_token: str = None):
    payload = {
        "id": lg_id,
        "name": lg.get("name"),
        "phone": lg.get("phone"),
        "zones": lg.get("zones", []),
        "online": lg.get("online", False),
        "last_seen": lg.get("last_seen"),
    }
    if session_token:
...
def zones():
    ...
    lg_id, _ = _resolve_lifeguard_token(request)
    if lg_id:
        items = _filter_zone_items_for_lifeguard(items, lg_id)
...
def zone_frame(zid: int):
    lg_id, _ = _resolve_lifeguard_token(request)
    if lg_id and not _zone_allowed_for_lifeguard(lg_id, zid):
        abort(403)
    ...
    try:
        ...
        with _crowd_lock:
            ...
            for alert in list(CROWD_ALERT_HISTORY):
                ...
                zone_name = _get_zone_display_name(zone_id) if zone_id is not None else "Unknown zone"
        ...
        return jsonify({
            ...
            "thresholds": {str(k): int(v) for k, v in CROWD_THRESHOLDS.items()},
        })
    except Exception as e:
        print(f"[crowd] crowd-alerts endpoint error: {e}")
...
def register_lifeguard():
    ...
    with _lifeguard_lock:
        ...
        for lg_id, lg in LIFEGUARDS.items():
            if phone and lg.get("phone") == phone:
                session_token = _issue_session(lg_id)
                lg["online"] = True
                lg["last_seen"] = time.time()
                _save_lifeguards()
                return jsonify({
                    **_lifeguard_public_profile(lg_id, lg, session_token),
    ...
    return jsonify({
        **_lifeguard_public_profile(lg_id, LIFEGUARDS[lg_id], session_token),
...
def login_lifeguard():
    """Lifeguard login by phone. Account must be created by admin (register)."""
    data = request.get_json() or {}
    phone = data.get("phone", "").strip()
    ...
    with _lifeguard_lock:
        lg_id = None
        lg = None
        for candidate_id, candidate in LIFEGUARDS.items():
            if candidate.get("phone") == phone:
                lg_id = candidate_id
                lg = candidate
                break
        ...
        if not lg_id:
            return jsonify({
                "error": "No account found for this phone. Ask your admin to create your account.",
            }), 404
    ...
    return jsonify(_lifeguard_public_profile(lg_id, lg, session_token))


@app.route("/api/lifeguards/me", methods=["GET"])
def lifeguard_me():
    """Return the logged-in lifeguard profile (Bearer token required)."""
    lg_id, _token = _resolve_lifeguard_token(request)
    if not lg_id:
    ...
    with _lifeguard_lock:
        ...
        return jsonify(_lifeguard_public_profile(lg_id, lg))


@app.route("/api/lifeguards/logout", methods=["POST"])
def logout_lifeguard():
    """Invalidate the current session token."""
    lg_id, token = _resolve_lifeguard_token(request)
    if not lg_id or not token:
        return jsonify({"error": "Unauthorized"}), 401

    with _lifeguard_lock:
        LIFEGUARD_SESSIONS.pop(token, None)
        if lg_id in LIFEGUARDS:
...
def get_lifeguard_alerts(lg_id: str):
    ...
    token_lg_id, _ = _resolve_lifeguard_token(request)
    if token_lg_id and token_lg_id != lg_id:
